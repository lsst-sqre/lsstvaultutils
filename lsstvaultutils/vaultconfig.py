...
class VaultConfig:
    ...
    def _change_secrets(self, verb: Verb, secret_name: str,
                        dry_run: bool = False) -> None:
        for name in self.enclaves:
            self._change_secret(verb=verb, enclave=self.enclaves[name],
                                secret_name=secret_name, dry_run=dry_run)

    def add_secret(self, enclave: Enclave, secret_name: str,
                   dry_run: bool = False) -> None:
        self._change_secret(verb=Verb.ADD, enclave=enclave,
                            secret_name=secret_name, dry_run=dry_run)

    def remove_secret(self, enclave: Enclave, secret_name: str,
                      dry_run: bool = False) -> None:
        self._change_secret(verb=Verb.REMOVE, enclave=enclave,
                            secret_name=secret_name, dry_run=dry_run)

    def _change_secret(self, verb: Verb, enclave: Enclave, secret_name: str,
                       dry_run: bool = False) -> None:
